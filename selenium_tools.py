...
def _get_driver() -> webdriver.Chrome:
    ...
    if _driver is None:
        ...
        # Try to ensure window appears on top
        ...
        # Use local ChromeDriver from C:\WebDriver
        logger.info("加载 ChromeDriver...")
        ...
        try:
            ...
            for _ in range(10):  # Try up to 10 times
                try:
                    ...
                    chrome_hwnd = ctypes.windll.user32.FindWindowW("Chrome_WidgetWin_1", None)
                    if chrome_hwnd:
                        break
                except:
                    pass
            ...
            if chrome_hwnd:
                # Method 1: Use AttachThreadInput to bypass foreground lock
                SW_RESTORE = 9
                SW_SHOW = 5
                ...
                if foreground_hwnd:
                    ...
                    # Attach input processing to the foreground thread
                    ctypes.windll.user32.AttachThreadInput(foreground_thread, current_thread, True)
                    
                    # Now we can set our window as foreground
                    ctypes.windll.user32.ShowWindow(chrome_hwnd, SW_RESTORE)
                    ctypes.windll.user32.SetForegroundWindow(chrome_hwnd)
                    ctypes.windll.user32.BringWindowToTop(chrome_hwnd)
                    
                    # Detach
                    ctypes.windll.user32.AttachThreadInput(foreground_thread, current_thread, False)
                else:
                    # Fallback: just show and bring to top
                    ctypes.windll.user32.ShowWindow(chrome_hwnd, SW_RESTORE)
                    ctypes.windll.user32.SetForegroundWindow(chrome_hwnd)
                    ctypes.windll.user32.BringWindowToTop(chrome_hwnd)
                
                logger.info("已将 Chrome 浏览器窗口置前")
            else:
                logger.debug("未找到 Chrome 窗口句柄")
        except Exception as e:
            logger.debug(f"无法将浏览器窗口置前: {e}")
